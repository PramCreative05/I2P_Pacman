...
static void ghost_move_script_FREEDOM_random(Ghost* ghost, Map* M) {
	//v TODO-HACKATHON 2-4: Uncomment the following code and finish pacman picking random direction.
	...
	ghost_NextMove(ghost, proba[generateRandomNumber(1, 4)]); //Lower Bound Upper Bound
	*/

	//v TODO-GC-random_movement: (Not in Hackathon) 
	// Description:
...
static void ghost_move_script_FREEDOM_shortest_path(Ghost* ghost, Map* M, Pacman* pman)
{
	...
	Directions K = shortest_path_direc(M, ghost->objData.Coord.x, ghost->objData.Coord.y, pman->objData.Coord.x, pman->objData.Coord.y);
	//ghost->objData.nextTryMove = K;
	...
	if(cnt >= 1) choice = generateRandomNumber(0, cnt-1);
	
	int prox = get_disc();
	if ( (prox >= 10 && prox <= 13) || ( prox >= 4 && prox <= 7) || ( prox >= 17 && prox <= 20) ) {
		ghost->objData.nextTryMove = (cnt >= 1) ? proba[choice] : counter_one;
		game_log("Ghost %d = %d", (int)ghost->typeFlag, prox);
	} 
	else {
		ghost->objData.nextTryMove = K;
	}
...
static void ghost_move_script_FLEE(Ghost* ghost, Map* M, const Pacman * const pacman) {
	//v TODO-GC-PB: escape from pacman
	// Description:
	...
	static Directions proba[4];
	uint32_t choice;
	Directions K = shortest_path_direc(M, ghost->objData.Coord.x, ghost->objData.Coord.y, pacman->objData.Coord.x, pacman->objData.Coord.y);

	int cnt = 0;
	for (Directions i = 1; i <= 4; i++)
		if ((i != K) && ghost_movable(ghost, M, i, !is_room_block(M, ghost->objData.Coord.x, ghost->objData.Coord.y))) 	proba[cnt++] = i;

	choice = (cnt > 1) ? generateRandomNumber(0, cnt-1) : 0;
	ghost_NextMove(ghost, proba[choice]);
}

/*---------------------------------FUNCTION BELOW IS TO UPDATE THE REAL COORD---------------------------------------*/
...
void ghost_move_script_shortest_path(Ghost* ghost, Map* M, Pacman* pacman) {
	//v TODO-GC-movement: do a little modification on shortest path move script
