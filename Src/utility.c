...
//v TODO-HACKATHON 3-5: Finish definition of pnt_in_rect
// Uncomment and fill in the code below.

bool pnt_in_rect(int px, int py, RecArea field) {
	int flag = 0;

	if((px > field.x && (px < (field.x + field.w))) && (py > field.y && (py < (field.y + field.h)))) flag = 1;
...
void parseCSV(const char *filename, char data[MAX_ROWS][MAX_COLS][MAX_NAME_LENGTH], int *numRows) {
	cfile = fopen(filename, "r");

	if (cfile == NULL) {
		game_abort("Error Opening Record");
	}
	else {
		char line[50];
		int ctr = 0;
		if (fgets(line, sizeof(line), cfile) != NULL) {
			...
			if (sscanf(line, "Entries [Name,Score,Time]: %d", numRows) == 1) {
				game_log("Total Rows Found");
				
				game_log("Ready to parse content");
				...
				while(ctr < MAX_ROWS && fgets(data[ctr][0], MAX_NAME_LENGTH, cfile) != NULL) {
					// game_log("Before: %s", data[ctr][0]);
					data[ctr][0][strcspn(data[0][0], "\n")] = '\0';
					// game_log("%s", data[ctr][0]);

					if (sscanf(data[ctr][0], "%s %d %f",
							name, &score, &timeValue) == 3) {
						// Injecting
						snprintf(data[ctr][1], sizeof(data[ctr][1]), "%s", name);
						snprintf(data[ctr][2], sizeof(data[ctr][2]), "%d", score);
						snprintf(data[ctr][3], sizeof(data[ctr][3]), "%f", timeValue);
						// Next
						ctr++;
					} else {
						game_abort("Fault parsing line at %d, Score: %d", ctr, score);
					}
				}
			}
			else {
		...
		for(int i = 0; i < ctr; i++) {
			for(int j = 0; j < ctr - 1; j++) {
				if (atoi(data[j][2]) > atoi(data[j + 1][2])) {
					char temp[MAX_NAME_LENGTH];
					strcpy(temp, data[j][1]);
					strcpy(data[j][1], data[j + 1][1]);
					strcpy(data[j + 1][1], temp);

					int tempScore = atoi(data[j][2]);
					strcpy(data[j][2], data[j + 1][2]);
					sprintf(data[j + 1][2], "%d", tempScore);

					float tempTime = atof(data[j][3]);
					strcpy(data[j][3], data[j + 1][3]);
					sprintf(data[j + 1][3], "%.2f", tempTime);
            	}
				else if(atoi(data[j][2]) == atoi(data[j + 1][2]) && atoi(data[j][3]) < atoi(data[j + 1][3])) {
					char temp[MAX_NAME_LENGTH];
					strcpy(temp, data[j][1]);
					strcpy(data[j][1], data[j + 1][1]);
					strcpy(data[j + 1][1], temp);
