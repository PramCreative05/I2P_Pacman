...
Map* create_map(const char * filepath) {
	...
	if (filepath == NULL) {
		M->row_num = 30;
		M->col_num = 36;
		game_log("Flag: Creating from default map. row = %d col = %d", M->row_num, M->col_num);
	}
	else {
		//v TODO-GC-read_txt: use fopen to open a file stream
		...
		//Read the row specification first
		if(fscanf(pFile, "%d %d", &M->row_num, &M->col_num) != 2) {
			game_abort("Map format unmatched\n");
			return NULL;
		}
		//Move the file cursor to the new line
		while(getc(pFile) != '\n'){};
		game_log("Ready to parse txt row = %d col = %d", M->row_num, M->col_num);
	}
	...
	*/
	...
	game_log("Malloc Complete: Start to parse");
	if(pFile == NULL) game_abort("open file error");
	...
	for (int i = 0; i < M->row_num; i++) {
		for (int j = 0; j < M->col_num; j++) {
			if (filepath == NULL){
				...
				//v TODO-GC-read_txt: input the map from file to M->map[row][col] 
				// '#' -> wall
				// '.' -> beans
				// 'B' -> room of ghost
				// 'P' -> Power Bean 
				if(fscanf(pFile, "%c", &M->map[i][j]) == 1) {
					game_log("Parsed: %c at row: %d col: %d", M->map[i][j], i, j);
				}
				else {
					game_abort("error parsing");
				}
...
void delete_map(Map* M) {
	...
	//v TODO-GC-memory: free the dynamic allocated part of Map* M at here;
	for (int i = 0; i < M->row_num; i++) {
		if (M->map[i]) {
			free(M->map[i]);
		}
		...
		for (int col = 0; col < M->col_num; col++) {
			switch (M->map[row][col])
			{
				case '#':
					...
					break;
				//v TODO-GC-PB: draw the power bean
				case 'P':
...
Directions shortest_path_direc(Map* M, int startGridx, int startGridy, int endGridx, int endGridy) {
	...
	static int8_t distances[MAX_WALL_NUM_H][MAX_WALL_NUM_W];
	...
	distances[startGridy][startGridx] = 0;
	end++;
	...
	while (front != end && steped[endGridy][endGridx] == NONE) {
		...
		for (size_t i = 0; i < 4; i++) {
			int8_t x = queue_x[front] + four_probe[i][0];
			int8_t y = queue_y[front] + four_probe[i][1];
			if (is_wall_block(M, x, y) || steped[y][x] || distances[y][x] != -1)
				continue;
			queue_x[end] = x;
			queue_y[end] = y;
			steped[y][x] = steped[queue_y[front]][queue_x[front]];
			distances[y][x] = distances[queue_y[front]][queue_x[front]] + 1;
			end++;
		}
		front++;
	}
	disc_temp = distances[endGridy][endGridx];
